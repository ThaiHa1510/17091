...
export class ReivewManagerComponent implements OnInit {
  ...
  updateReviewStatus(review: any, isApproved: boolean) {
    const data = {
      id: review._id,
      isApproved
    };
    this.reviewService.updateReviewStatus(data).subscribe((data: any) => {
      if (data && data.status === true) {
        const i = this.listReviews.data.findIndex((c: any) => c._id == review._id);
        this.listReviews.data[i]['isApproved'] = isApproved;
      }
    });
  }
}
