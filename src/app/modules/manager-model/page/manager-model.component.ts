...
import { ModelService, AlertService } from '@/_servies';
import { Model, EventSocket } from '@/_models';
...
export class ManagerModelComponent implements OnInit ,OnDestroy{
  ...
  assigned: string = '';
  displayedColumns: string[] = ['select', 'email', 'name', 'role', 'ipAddress', 'status','isCalling', 'token', 'actions'];
  ...
  listernOnline() {
    this.subscriptions.add(this.modelService.socketService.onEvent(this.modelService.EventModelOnline).subscribe(data => {
      let listModels = this.listModels.data;
      listModels.forEach((item, index) => {
        if (item.id == data) {
          listModels[index].status = true;
        }
      })
      this.listModels.data = listModels;
    }));
  }
  listernOffline() {
    this.subscriptions.add(this.modelService.socketService.onEvent(this.modelService.EventModelOffline).subscribe(data => {
      let listModels = this.listModels.data;
      listModels.forEach((item, index) => {
        if (item.id == data) {
          listModels[index].status = false;
        }
      })
      this.listModels.data = listModels;
    }));
  ...
  listernCalling() {
    this.subscriptions.add(this.modelService.socketService.onEvent(EventSocket.CALLSTART).subscribe(data => {
      console.log('listernCalling');
      let listModels = this.listModels.data;
      listModels.forEach((item, index) => {
        if (item.id == data) {
  ...
  listernCallingOff() {
    this.subscriptions.add(this.modelService.socketService.onEvent(EventSocket.CALLEND).subscribe(data => {
      console.log('listernCallingOff')
      let listModels = this.listModels.data;
      listModels.forEach((item, index) => {
        if (item.id == data) {
