...
export class DashboardService {
  private currentListOrderSubject: BehaviorSubject<any> = new BehaviorSubject<any>(
    null
  );
  public currentListOrder = this.currentListOrderSubject.asObservable();
  private currentListUserSubject: BehaviorSubject<IUser> = new BehaviorSubject<IUser>(
    null
  );
  public currentListUser = this.currentListUserSubject.asObservable();
  private alartService: AlertService;
  public reports: any;
  private currentListPayoutSubject: BehaviorSubject<any> = new BehaviorSubject<any>(
    null
  );
  public currentListPayout = this.currentListOrderSubject.asObservable();
  private currentListModelSubject: BehaviorSubject<IModel> = new BehaviorSubject<IModel>(
    null
  );
  ...
  constructor(
    ...
    userService.getAllUserasAdmin().subscribe((data) => {
      this.currentListUserSubject.next(data);
    });
    modelService.getAllModelasAdmin().subscribe((data) => {
  ...
  public getReports() {
    return this.httpClient.get<any>(
      `${environment.apiUrl}/api/backend/reports`
    );
  }

  public getCharts() {
    return this.httpClient.get<any>(
      `${environment.apiUrl}/api/backend/reports/charts`
    );
  }
  ...
  getListOrder(
    ...
    const queryUrl = `?&sort=${sort}&order=${order}&page=${
      page + 1
    }&limit=${limit}`;
    return this.httpClient
      .get<any>(`${environment.apiUrl}/api/backend/order/${queryUrl}`)
      .pipe(
        map((data) => {
  ...
  getListPayout(
    ...
    return this.httpClient.get<any>(
      `${environment.apiUrl}/api/backend/payout/`
    );
  }
}
