...
export class RegisterModelComponent implements OnInit {
  ...
  constructor(private alertService: AlertService, private _formBuilder: FormBuilder, private httpClient: HttpClient
    ...
    , private ipServiceService: IpServiceService,private translateService:TranslateService) {
  ...
  onSubmit() {
    ...
    if (this.loading) {
      ...
      let email = this.registerFormGroup.get('emailCtrl').value;
      this.user.ipAddress = this.ipAddress;
      this.userService.registerModel(this.fileData, this.user)
        .subscribe(data => {
          if (data.status == true) {
            this.loading = false;
            this.alertService.success(this.translateService.instant('To confirm your registration, please check the link we sent you on your email address')+" '" + email + '"', true, false,15000);
