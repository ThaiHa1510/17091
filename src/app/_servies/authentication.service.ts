...
export class AuthenticationService {
  ...
  public isLoginSubject = new BehaviorSubject(null);
  public isLogin: Observable<any> = this.isLoginSubject.asObservable();
  ...
  constructor(private http: HttpClient, private router: Router, private localStorage: LocalstorageService) {
    LocalstorageService.isBrowser.subscribe(data => {
      if (data) {
        ...
        if (user) {
          this.isLoginSubject = new BehaviorSubject(true);
        } else {
          this.isLoginSubject = new BehaviorSubject(false);
        }
  ...
  public setCurrentUserValue(value) {
    this.currentUserSubject.next(new User().deserialize(value));
  }
